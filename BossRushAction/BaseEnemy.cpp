...
BaseEnemy::BaseEnemy()
{
	...
	isBehind = false;
	hpbarPos = { lpScene.GetScreenSize().x / 2 + 200,30 };
	deadFlag = false;
...
Vector2 BaseEnemy::DistanceCalcurator(Vector2 objectPos)
{
	return (objectPos - pos);
}

bool BaseEnemy::IsHitObject(Vector2 distance,const Vector2& size)
{
	ClampDistance(distance);
	//if (!attackFlag)
	{
		if (distance.x <= (collider.enemyAnimationRectData[myAnimationType].enemyFrameRectData[animationID].size[R_State_Damage][0].x/2 + size.x / 2) &&
			distance.y <= (collider.enemyAnimationRectData[myAnimationType].enemyFrameRectData[animationID].size[R_State_Damage][0].y/2 + size.y / 2))
		{
...
bool BaseEnemy::IsAttack(const Vector2& playerRectSize)
{
	ClampDistance(distance);
	// if player is into sight range....
	if(distance.x <= (playerRectSize.x / 2) + (rectSize.x / 2)
		&& distance.y <= (playerRectSize.y / 2) + (rectSize.y / 2))
	{
		attackFlag = true;
		intoSightRangeFlag = false;
	}
