...
// hp
#define MAXHP 10
// number of super attack gauge max
#define GaugeMax 150
...
public:
	...
	void DistanceCalcurator(Vector2 enemyPos);
	// set animation
	...
	bool CheckBehind(const Vector2& enemyPos);

	const ActionRect& GetColliderRect(void)
	{
		return collider[myAnimationType].playerAnimLevelData[animLevelCount[myAnimationType]].playerRectData[animationID];
	}
	...
	const ENEMYTYPE& GetisHitEnemyType(void)
	{
		return isHitEnemyType;
	}
	...
	bool& GetAttackJumpFlag(void)
	{
		return attackJumpFlag;
	}

	// you can get shot information
	SharedShotListPtr& GetShot(void)
	{
		return shotList;
	}
...
private:
	...
	void DrawSuperAttackGauge(void);
...
protected:
	...
	// the farest enemy position
	Vector2 farestEnemyPos;

	Vector2 distance;

	ENEMYTYPE isHitEnemyType;

	// you can go next attack motion "sword" or "kick"
	bool cannextAttackFlag;

	// 0:kick 1:sword
	bool nextAttackFlag[2];
	...
	bool attackMotionFlag;

	// shot
	SharedShotListPtr shotList;
	bool shotFlag;
