...
// create player list
using SharedShotPtr = shared_ptr<Shot>;
using SharedShotList = list<SharedShotPtr>;
using SharedShotListPtr = shared_ptr<SharedShotList>;

using WeakShotList = weak_ptr<SharedShotList>;
using SharedShotListItr = SharedShotList::iterator;
...
struct AddShotList
{
	SharedShotListItr operator()(WeakShotList shotList, SharedShotPtr shotPtr)
	{
		shotList.lock()->push_back(shotPtr);
		SharedShotListItr shotItr = shotList.lock()->end();;
		shotItr--;
